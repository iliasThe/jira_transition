...
jira = JIRA(options=jira_options, basic_auth=('логин','пароль'))
...
def collect_task(jql, amount: int):
    cnt_task = 0
    print(bot.send_message(chat_id='', text=f'Начало скрипта, кол-во: {amount} -' + f' {datetime.now()}'))
    while cnt_task <=amount:
        ...
        for keyid in range(len(jira_key)):
            ...
            issue = jira.issue(key_name)
            # апдейт исполнителя
            issue.update(assignee={'name': 'логин'})
            # вывод
            print(issue.fields.assignee, key_name)
            cnt_task = cnt_task + 1
            time.sleep(3)
    print(bot.send_message(chat_id='', text = 'Конец скрипта ' + f'{datetime.now()}'))
